  ...
  renderLazyLoadEvents: function renderLazyLoadEvents(data) {
    ...
    data.forEach(function (element) {
      ...
      if (element.type === 'Tur') {
        ...
        tourType = 'tourBus';
      } else if (element.type === 'pendiente') {
          tourType = 'pendiente';
      } else if (element.type === 'lineBad') {
          tourType = 'lineBad';
      } else if (element.type === 'proceso') {
          tourType = 'proceso';
      } else if (element.type === 'entregado') {
          tourType = 'entregado';
      } else {
        tourType = 'cruise';
      }

      var title = element.minNight + ' Dia(s)';
  ...
  createClass(Gantt, [{
    ...
    value: function renderEvents(num) {
      ...
      data.forEach(function (el) {
        ...
        if (el.type === 'Tur') {
          ...
          tourType = 'tourBus';
        } else if (el.type === 'pendiente') {
            tourType = 'pendiente';
        } else if (el.type === 'lineBad') {
            tourType = 'lineBad';
        } else if (el.type === 'proceso') {
            tourType = 'proceso';
        } else if (el.type === 'entregado') {
            tourType = 'entregado';
        } else {
          tourType = 'cruise';
        }

        var title = el.minNight + ' Dia(s)';
    ...
    value: function tooltipView(data) {
        var template = '' +
            ...
            '        <span>Inicio: <span class="desc">{5}</span></span><br>' +
            '        <span>Fin: <span class="desc">{6}</span></span>' +
            '        <div class="price">' +
            '            <div class="tl">{7}</div>' +
            '            <div class="eur">{8}</div>' +
            '        </div>' +
