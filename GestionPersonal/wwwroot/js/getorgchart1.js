    ...
    if (a) {
        for (var e in this.config) {
            if (typeof a[e] === "object" && !Array.isArray(a[e])) {
                this.config[e] = BALKANGraph._ag(this.config[e], a[e])
            } else {
...
OrgChart.prototype.linkNode = function(b, j) {
    if (this._g(b, j)) {
        ...
        if (c.isChildOfGroup) {
            this.removeNodeTag(c.id, c._4);
            var d = this.getBGNode(c._4);
            if (d.bgnodes.length == 2) {
                for (var e = 0; e < d.bgnodes.length; e++) {
                    var a = d.bgnodes[e];
                    if (c.id != a.id) {
                        this.removeNodeTag(a.id, a._4)
                    }
                }
            }
        }
...
OrgChart.prototype.removeNode = function(b, a) {
    ...
    this._l(false, BALKANGraph.action.update, null, function() {
        BALKANGraph._au(c.getSvg(), c.getViewBox(), c.response.boundary)
    })
};
...
OrgChart.prototype._aY = function(a, b, f) {
    var d = JSON.parse(JSON.stringify(this.config.tags));
    d[a][b] = f;
...
OrgChart.prototype._aA = function(d, b) {
...
OrgChart.prototype.group = function(h, l) {
    ...
    if (!g.isGroup && !k.isGroup && !g.isChildOfGroup && !k.isChildOfGroup) {
        ...
        if (!g.isGroup && !k.isGroup && g.isChildOfGroup && !k.isChildOfGroup) {
            ...
            this._aA(g._4, g.id);
            this.removeNodeTag(g.id, g._4, true);
            this.addNodeTag(g.id, c, true);
            this.addNodeTag(k.id, c, true);
            var f = this._S(k.id).pid;
            if (g.id != f) {
            ...
            if (!g.isGroup && !k.isGroup && !g.isChildOfGroup && k.isChildOfGroup) {
                ...
                if (!g.isGroup && !k.isGroup && g.isChildOfGroup && k.isChildOfGroup) {
                    this._aA(g._4, g.id);
                    this.removeNodeTag(g.id, g._4, true);
                    this.addNodeTag(g.id, k._4, true)
                }
            }
        }
    }
...
OrgChart.prototype._an = function(b, a, c) {
    c[0] = a;
    c[1] = b;
    this.setViewBox(c);
    this.xScrollUI.setPosition();
...
BALKANGraph.RES = {};
BALKANGraph.RES.IT_IS_LONELY_HERE_LINK = "Agrega tu primer nodo.";
...
OrgChart.prototype._ay = function(k, c, g) {
    var m = this;
    ...
    var f = function(t) {
        ...
        if (l && l.type == "pan") {
            ...
            if (l && l.type == "pinch") {
                ...
                l.dist = s;
                var v = BALKANGraph._ak(m.element, m.width(), m.height(), t);
                m.zoom(x, v)
            }
        }
    };
    var d = function() {
        if (l.type == "pan") {
            BALKANGraph._au(k, m.getViewBox(), m.response.boundary, function() {
...
OrgChart.editUI.prototype.hide = function(f) {
    ...
    if (f) {
        ...
        var g = this;
        if (this.node.isGroup) {
            this.obj._aY(this.node.id, "groupName", b.Name)
        } else {
...
OrgChart.ui = {
    ...
    defs: function() {
        ...
        for (var c in OrgChart.templates) {
            ...
            if (b.defs) {
                OrgChart.ui._i[c] = BALKANGraph._al();
    ...
    node: function(c, e, b, d, u, v, k, a) {
        ...
        if (c.isGroup) {
            ...
            if (i) {
                ...
                h = h.replaceAll("{randId}", BALKANGraph._al()).replaceAll("{randId2}", BALKANGraph._al());
                l += h
            }
            l += h
        } else {
            for (var g in k) {
                ...
                if (e) {
                    ...
                    if (p != undefined && p != null && r[g] != undefined) {
                        var h;
                        if (!BALKANGraph._as(d, g) && typeof(p) == "string") {
                            p = BALKANGraph._aJ(p, r[g]);
                            h = r[g].replace("{val}", p)
                        } else {
                            h = r[g].replace("{val}", p)
                        }
        ...
        var j = OrgChart.ui._aZ(c, r, d, b, k, u, v, a);
        ...
        for (var g = 0; g < a.children.length; g++) {
            ...
            if ((b.isAssistant || b.layout == 2) && b.rightNeighbor && b.rightNeighbor.templateName == "split") {
                switch (c.orientation) {
                    ...
                    case BALKANGraph.orientation.right:
                    case BALKANGraph.orientation.right_top:
                        m = OrgChart.ui._ax(b.rightNeighbor, b, r, q);
                        break;
                    case BALKANGraph.orientation.left:
                    case BALKANGraph.orientation.left_top:
                        m = OrgChart.ui._ax(b.rightNeighbor, b, r, q);
                ...
                if ((b.isAssistant || b.layout == 2) && b.leftNeighbor && b.leftNeighbor.templateName == "split") {
                    switch (c.orientation) {
                        case BALKANGraph.orientation.top:
                        case BALKANGraph.orientation.top_left:
                            m = OrgChart.ui._ae(b.leftNeighbor, b, r, q);
                            break;
                        case BALKANGraph.orientation.bottom:
                        case BALKANGraph.orientation.bottom_left:
                            m = OrgChart.ui._ae(b.leftNeighbor, b, r, q);
                        ...
                        if (p === "edit") {
                            ...
                            if (p === "details") {
                                ...
                                if (p === "remove") {
                                    ...
                                    if (p === "svg") {
                                        l.obj.exportSVG("Organigrama.svg", false, b)
                                    } else {
                                        if (p === "pdf") {
                                            l.obj.exportPDF("Organigrama.pdf", false, b)
                                        } else {
                                            if (p === "png") {
                                                l.obj.exportPNG("Organigrama.png", false, b)
                                            } else {
                                                if (p === "csv") {
                                                    l.obj.exportCSV()
                                                } else {
...
BALKANGraph.toolbarUI.prototype.init = function(b, d) {
    this.obj = b;
    this.toolbar = d;
    if (d) {
        this._aN = false;
        this.div = document.createElement("div");
        this.div.innerHTML = '<div style="float:right;"><svg data-tlbr="layout" style="cursor: pointer;padding-right: ' + this.obj.config.padding + "px; padding-bottom: " + this.obj.config.padding + 'px" width="36px" height="36px" viewBox="0 0 220.682 220.682"><polygon fill="#757575" points="110.341, 0 10.639, 99.703 38.924, 127.986 110.341, 56.568 181.758, 127.986 210.043, 99.703 	"/><polygon fill="#757575" points="110.341, 92.695 10.639, 192.396 38.924, 220.682 110.341, 149.264 181.758, 220.682 210.043, 192.396 	"/></svg></div><div  style="clear: both;"></div><div data-layouts="" style="background-color: #f9f9f9;height: 100%; padding-top:20px;"><svg data-layout="normal" style="cursor: pointer;" width="110" height="100"><rect fill="#039BE5" x="35" y="0" width="50" height="27"></rect><rect fill="#F57C00" x="7" y="41" width="50" height="27"></rect><rect fill="#F57C00" x="63" y="41" width="50" height="27"></rect><line stroke="#000000" x1="60" x2="60" y1="27" y2="35" stroke-width="1"></line><line stroke="#000000" x1="32" x2="88" y1="35" y2="35" stroke-width="1"></line><line stroke="#000000" x1="32" x2="32" y1="35" y2="41" stroke-width="1"></line><line stroke="#000000" x1="88" x2="88" y1="35" y2="41" stroke-width="1"></line></svg><svg data-layout="treeRightOffset" style="cursor: pointer;" width="110" height="100"><rect fill="#039BE5" x="35" y="0" width="50" height="27"></rect><rect fill="#F57C00" x="40" y="41" width="50" height="27"></rect><rect fill="#F57C00" x="40" y="73" width="50" height="27"></rect><line stroke="#000000" x1="60" x2="60" y1="27" y2="35" stroke-width="1"></line><line stroke="#000000" x1="60" x2="35" y1="35" y2="35" stroke-width="1"></line><line stroke="#000000" x1="35" x2="35" y1="35" y2="86" stroke-width="1"></line><line stroke="#000000" x1="35" x2="40" y1="86" y2="86" stroke-width="1"></line><line stroke="#000000" x1="35" x2="40" y1="54" y2="54" stroke-width="1"></line></svg><svg data-layout="treeLeftOffset" style="cursor: pointer;" width="110" height="100"><rect fill="#039BE5" x="35" y="0" width="50" height="27"></rect><rect fill="#F57C00" x="30" y="41" width="50" height="27"></rect><rect fill="#F57C00" x="30" y="73" width="50" height="27"></rect><line stroke="#000000" x1="60" x2="60" y1="27" y2="35" stroke-width="1"></line><line stroke="#000000" x1="60" x2="85" y1="35" y2="35" stroke-width="1"></line><line stroke="#000000" x1="85" x2="85" y1="35" y2="86" stroke-width="1"></line><line stroke="#000000" x1="80" x2="85" y1="86" y2="86" stroke-width="1"></line><line stroke="#000000" x1="80" x2="85" y1="54" y2="54" stroke-width="1"></line></svg><svg data-layout="mixed" style="cursor: pointer;" width="110" height="100"><rect fill="#039BE5" x="35" y="0" width="50" height="27"></rect><rect fill="#F57C00" x="35" y="41" width="50" height="27"></rect><rect fill="#F57C00" x="35" y="73" width="50" height="27"></rect><line stroke="#000000" x1="60" x2="60" y1="27" y2="41" stroke-width="1"></line><line stroke="#000000" x1="60" x2="60" y1="68" y2="73" stroke-width="1"></line></svg><svg data-layout="tree" style="cursor: pointer;" width="110" height="100"><rect fill="#039BE5" x="35" y="0" width="50" height="27"></rect><rect fill="#F57C00" x="7" y="41" width="50" height="27"></rect><rect fill="#F57C00" x="7" y="73" width="50" height="27"></rect><rect fill="#F57C00" x="63" y="41" width="50" height="27"></rect><rect fill="#F57C00" x="63" y="73" width="50" height="27"></rect><line stroke="#000000" x1="60" x2="60" y1="27" y2="86" stroke-width="1"></line><line stroke="#000000" x1="57" x2="63" y1="54" y2="54" stroke-width="1"></line><line stroke="#000000" x1="57" x2="63" y1="86" y2="86" stroke-width="1"></line></svg></div>';
        ...
        var a = this.div.querySelector("[data-layouts]");
        var c = this;
...
BALKANGraph.toolbarUI.prototype.showLayout = function() {
    this._aN = true;
    this.layoutBtn.style.transform = "rotate(180deg) translateX(36px) translateY(36px)";
...
OrgChart.server.prototype.read = function(j, o, d, n, a, b) {
    ...
    var k = OrgChart.server._M(o, d, this.visibleNodes, this.config, e, f, g, h, this.viewBox, m, this.action, this.actionParams, i, c, this.oldNodes);
    if (a != BALKANGraph.action.exporting) {
        this.maxX = e;
        this.maxY = f;
        this.minX = g;
...
OrgChart.server.prototype._ap = function() {
    ...
    for (var b = 0; b < k.length; b++) {
        ...
        OrgChart.server._aS(k[b], 0, 0, 0, d, e, this.config)
    }
...
OrgChart.server.prototype.search = function(n) {
    ...
    for (var e = 0; e < d.length; e++) {
        ...
        var p = BALKANGraph._O(d[e].tags, this.config.tags, this.config.template);
        m.push({
            id: d[e].id,
...
OrgChart.server._m = function(n, a, b, I) {
    ...
    for (var y = 0; y < q.length; y++) {
        var p = q[y];
        var R = Array.isArray(p.tags) ? p.tags.slice(0) : [];
        var s = BALKANGraph._F(R, n.tags);
        var T = BALKANGraph._O(R, n.tags, n.template);
        var P = Array.isArray(p.spids) ? p.spids : null;
        var D = new BALKANGraph.node(p.id, p.pid, P, R, T);
        D.isAssistant = (R.indexOf("assistant") != -1);
    ...
    for (var y = 0; y < E.length; y++) {
        ...
        if (H) {
            ...
            OrgChart.server._aV(D, H.groupState, n.orientation)
        } else {
            OrgChart.server._aV(D, BALKANGraph.EXPAND, n.orientation)
        }
        OrgChart.server._0(D, F, a, n, b, u, m);
        OrgChart.server._7(D, F, a, n, b)
    }
    for (var y = 0; y < f.length; y++) {
        var J = F[f[y]];
        var N = new BALKANGraph.node(J.id + "_split_0", J.id, null, [], "split");
        F[N.id] = N;
        var H = I[N.id];
        if (H) {
            F[N.id].state = H.state;
            if (F[N.id].state == BALKANGraph.COLLAPSE) {
                m.push(N.id)
            }
        }
        OrgChart.server._0(N, F, a, n, b, u, m);
        var e = [];
        for (var B = J.children.length - 1; B >= 0; B--) {
            ...
            if (l.isAssistant) {
                ...
                l.parent = N;
                N.children.push(l);
                N.childrenIds.push(l.id);
                J.children.splice(B, 1)
            }
        }
        if (e.length % 2) {
            var C = F[e[e.length - 1]];
            var Q = OrgChart.templates[C.templateName];
            var U = "transperant_" + C.templateName;
            OrgChart.templates[U] = {
                linkAdjuster: {},
                link: "",
                node: "",
                nodeMenuButton: "",
            ...
            var V = new BALKANGraph.node(J.id + "_transperant", C.pid, null, [], U);
            V.isAssistant = true;
            F[V.id] = V;
            var H = I[V.id];
            if (H) {
                F[V.id].state = H.state;
                if (F[V.id].state == BALKANGraph.COLLAPSE) {
                    m.push(V.id)
                }
            }
            OrgChart.server._0(V, F, a, n, b, u, m);
            e.splice(e.length - 1, 0, V.id)
        }
        var g = 1;
        for (var B = e.length - 1; B >= 0; B--) {
            if (B % 2 && B != e.length - 1) {
                var O = new BALKANGraph.node(J.id + "_split_" + g, null, null, [], "split");
                F[O.id] = O;
                var H = I[O.id];
                if (H) {
                ...
                OrgChart.server._0(O, F, a, n, b, u, m);
                e.splice(B, 0, O.id);
                g++
            } else {
                if (B % 2) {
        ...
        for (var B = 0; B < e.length; B = B + 3) {
            ...
            K.children.push(M);
            K.children.push(S);
            K.childrenIds.push(r.id);
            K.childrenIds.push(M.id);
            K.childrenIds.push(S.id)
        }
    }
    if (a == BALKANGraph.action.centerNode) {
        var k = F[b.id];
        if (!k) {
            k = F[u[b.id]];
            OrgChart.server._aV(k, BALKANGraph.EXPAND, n.orientation)
        }
            ...
            for (var B = J.children.length - 1; B >= 0; B--) {
                if (J.children[B] == D) {
                    ...
                    if (D.templateName == "split" || J.templateName == "split") {
                        var K = D.parent;
                        if (K) {
                            while (K.templateName == "split") {
                                K = K.parent
                            }
                            for (var G = 0; G < K.childrenIds.length; G++) {
                                var c = K.childrenIds[G];
                                if (K.collapsedChildrenIds.indexOf(c) == -1) {
                                    K.collapsedChildrenIds.push(c)
                                }
                            }
                        }
                    } else {
            ...
            for (var B = J.children.length - 1; B >= 0; B--) {
                ...
                if (l.childrenIds.length == 0 && !l.isAssistant && l.templateName != "split") {
                    ...
                    if (n.layout == BALKANGraph.treeLeftOffset || n.layout == BALKANGraph.treeRightOffset) {
                        var U = "transperant_" + l.templateName;
                        OrgChart.templates[U] = {
                            linkAdjuster: {},
                            link: "",
                            node: "",
                        ...
                        var V = new BALKANGraph.node(l.id + "_transperant", null, null, [], U);
                        V.layout = BALKANGraph.tree;
                        F[V.id] = V;
                        W.splice(0, 0, V.id)
                    }
                    if (n.layout != BALKANGraph.treeRightOffset) {
...
OrgChart.server._0 = function(k, l, a, d, b, f, c) {
...
OrgChart.server._7 = function(e, f, a, c, b) {
    if (a == BALKANGraph.action.exporting && b.expandChildren == true && e) {
        OrgChart.server._aV(e, BALKANGraph.EXPAND, c.orientation);
        return
    } else {
        if (a == BALKANGraph.action.init && e) {
            if (e.isGroup) {
                var d = c.tags[e.id].groupState;
                OrgChart.server._aV(e, d, c.orientation)
            }
        }
    }
    ...
    OrgChart.server._aV(f[b.id], b.state, c.orientation)
};
OrgChart.server._aX = function(e, f) {
    if (e.isGroup && e.groupState == BALKANGraph.EXPAND) {
        ...
        if (k.layout_type == 0) {
            ...
            for (var d = 0; d < e.bgnodes.length; d++) {
                var b = e.bgnodes[d];
                b.x = e._5._y / 2 - b.w / 2 + k.groupNodesSeparation + (e._5._y * a + k.groupNodesSeparation * a);
                b.y = e._5._b / 2 - b.h / 2 + k.groupNodesSeparation + (e._5._b * j + k.groupNodesSeparation * j);
                if (a >= e._5._n - 1) {
                    a = 0;
                    j++
                } else {
...
OrgChart.server._9 = function(j, o, k) {
...
OrgChart.server._aV = function(a, c, b) {
    a.groupState = c;
    OrgChart.server._9(a, a.templateName, b)
};
...
OrgChart.server._aw = function(e, b, f, a) {
...
OrgChart.server._c = function(n, h, a) {
    ...
    for (var e = BALKANGraph.MAX_DEPTH - h; b != null && c != null && d <= e;) {
        ...
        var u = (c._ao + i + OrgChart.server._Y(c, a) + t) - (b._ao + m);
        if (u > 0) {
            var r = n;
            var o = 0;
...
OrgChart.server._M = function(K, n, D, j, r, s, t, u, H, k, c, d, A, l, C) {
    var I = {};
    var f = {
        top: null,
        left: null,
    ...
    switch (j.orientation) {
        ...
        case BALKANGraph.orientation.left_top:
            if (s < n / F) {
                ...
                f.top = s + j.padding - n / F
            }
            if (r < K / F) {
                f.right = t - j.padding;
                f.left = (r + j.padding - K / F)
            }
            break;
        case BALKANGraph.orientation.bottom:
        case BALKANGraph.orientation.bottom_left:
            if (Math.abs(u) < n / F) {
                f.bottom = u - j.padding;
                f.top = Math.abs(s) + j.padding - n / F
            }
            if (r < K / F) {
                f.right = t - j.padding;
                f.left = r + j.padding - K / F
        ...
        case BALKANGraph.orientation.right_top:
            ...
            if (Math.abs(t) < K / F) {
                f.right = t - j.padding;
                f.left = Math.abs(r) + j.padding - K / F
            }
            break
    }
    ...
    for (var o in A) {
        var z = A[o];
        if (OrgChart.server._aw(z, j, H, c)) {
            I[z.id] = z;
            var b = null;
...
OrgChart.server._aD = function(c, a, b) {
    if (b[a] == null) {
        b[a] = 0
    }
    if (b[a] < c.h) {
...
OrgChart.server._aC = function(c, a, b) {
    if (b[a] == null) {
        b[a] = 0
    }
    if (b[a] < c.w) {
...
OrgChart.server._aF = function(b, a, c) {
    if (c[a] && c[a].id === b.id) {
        return
    }
    b.leftNeighbor = c[a];
...
OrgChart.server._aS = function(h, c, l, n, e, f, a) {
    if (c <= BALKANGraph.MAX_DEPTH) {
        ...
        if (OrgChart.server._E(h) != 0) {
            ...
            OrgChart.server._aS(OrgChart.server._R(h), c + 1, l + h._ab, n + g + d, e, f, a)
        }
        var j = OrgChart.server._I(h);
        if (j != null) {
            OrgChart.server._aS(j, c, l, n, e, f, a)
        }
    }
};
OrgChart.server._W = function(m, g, h, j, o, a) {
    ...
    OrgChart.server._aD(m, g, h);
    OrgChart.server._aC(m, g, j);
    OrgChart.server._aF(m, g, o);
    OrgChart.server._aX(m, a.orientation);
    if (OrgChart.server._E(m) == 0 || g == BALKANGraph.MAX_DEPTH) {
        ...
        if (f != null) {
            ...
            m._ao = f._ao + OrgChart.server._Y(f, a) + p
        } else {
            m._ao = 0
        }
    } else {
        ...
        var k = b._ao + ((e._ao - b._ao) + OrgChart.server._Y(e, a)) / 2;
...
OrgChart.server._aR = function(c, a) {
...
OrgChart.prototype._af = function(b) {
    this.nodeMenuUI.hide();
    this.dragDropMenuUI.hide();
    this.menuUI.hide();
    var a = this.getBGNode(b);
    ...
    if (c.nodeBinding) {
        for (var a in c.nodeBinding) {
            if (c.nodeBinding[a] == d) {
                return BALKANGraph._as(c, a)
            }
        }
    }
    return false
};
BALKANGraph._6 = function() {
    function a() {
...
BALKANGraph._au = function(e, d, a, b) {
    var c = d.slice(0);
...
BALKANGraph._q = function(a) {
    return '<a style="text-decoration: none;position:absolute;bottom:' + a + "px;left:" + a + 'px;" title="splittel.com" href="#"></a>'
};
BALKANGraph._al = function() {
...
BALKANGraph._ak = function(b, m, c, a) {
    var d = BALKANGraph._L(b);
...
BALKANGraph._at = function(b, a, d) {
    var g = a.offsetX;
    var h = a.offsetY;
...
OrgChart.templates.ula.field_0 = '<text width="230" class="field_0" style="font-size: 18px;" fill="#DA4453" x="60" y="55">{val}</text>';
OrgChart.templates.ula.field_1 = '<text width="230" class="field_1" style="font-size: 12px;" fill="#626567" x="5" y="76">{val}</text>';
OrgChart.templates.ula.node = '<rect x="0" y="0" height="120" width="250" fill="#ffffff" stroke-width="1" stroke="#aeaeae"></rect><line x1="0" y1="0" x2="250" y2="0" stroke-width="1" stroke="#DA4453"></line>';
...
OrgChart.templates.ula.menu = '<g style="cursor:pointer;" transform="matrix(1,0,0,1,225,12)" control-node-menu-id="{id}"><rect x="-4" y="-10" fill="#ffffff" width="30" height="22"></rect><circle cx="0" cy="0" r="2" fill="#039BE5"></circle><circle cx="7" cy="0" r="2" fill="#039BE5"></circle><circle cx="14" cy="0" r="2" fill="#039BE5"></circle></g>';
...
OrgChart.templates.olivia.field_0 = '<text width="145" class="field_0" style="font-size: 18vw;" fill="#757575" x="100" y="55">{val}</text>';
OrgChart.templates.olivia.field_1 = '<text width="145" class="field_1" style="font-size: 14vw;" fill="#757575" x="100" y="76">{val}</text>';
...
OrgChart.prototype._v = function(a) {
    this._a(window, "resize", this._aW)
};
OrgChart.prototype._f = function(b) {
    var b = this.getSvg();
    if ("ontouchstart" in window && navigator.maxTouchPoints) {
        this._a(b, "touchstart", this._2);
        this._a(b, "touchend", this._1)
    } else {
        this._a(b, "mousedown", this._2);
        this._a(b, "click", this._1);
        if (this.config.mouseScroolBehaviour == BALKANGraph.action.zoom) {
            this._a(b, "DOMMouseScroll", this._ah);
            this._a(b, "mousewheel", this._ah)
        }
    }
...
OrgChart.prototype._aQ = function(a, b) {
...
OrgChart.prototype._1 = function(i, a) {
    if (a.type.indexOf("touch") != -1 && this._aG && a.changedTouches.length == 1) {
        if (a.changedTouches.length) {
            var j = {
                x: a.changedTouches[0].clientX,
                y: a.changedTouches[0].clientY
            ...
            var h = this.getScale();
            var c = (Math.abs(j.x - this._aG.x) / h);
            var d = (Math.abs(j.y - this._aG.y) / h);
            this._aG = null;
            if (c > g[0] / 10) {
                return
            }
        ...
        if (a.type.indexOf("touch") != -1 && this._aG == null) {
            return
        }
    }
    var f = a.target;
...
OrgChart.prototype._ah = function(c, b) {
    var d = this;
    var a = b.wheelDelta ? b.wheelDelta / 40 : b.detail ? -b.detail : 0;
    var f = function(e) {
        var h = e > 0;
        var g = BALKANGraph._h(d.getSvg(), b.pageX, b.pageY);
        d.zoom(h, g)
    };
...
OrgChart.prototype._ai = function(p, k, m) {
    ...
    c.style.opacity = 0.7;
    if (a.isChildOfGroup) {
        var n = this.getNodeElement(a._4);
        var e = BALKANGraph._P(n);
        s = s + e[4];
        t = t + e[5];
        j[4] = s;
        j[5] = t;
    ...
    var l = function(v) {
        if (d) {
            ...
            if (h != null) {
                var x = r.getBGNode(h);
                if (x.isChildOfGroup) {
                    r.getNodeElement(x._4).style.opacity = 1;
                    g.style.opacity = 1
                } else {
                    if (!x.isGroup) {
                        g.style.opacity = 1
            ...
            if (h != null) {
                var x = r.getBGNode(h);
                if (x.isChildOfGroup) {
                    r.getNodeElement(x._4).style.opacity = 0.8;
                    g.style.opacity = 0.5
                } else {
                    if (x.isGroup) {
                        h = null;
    ...
    var i = function(v) {
        ...
        if ((a.id == h) || (a.isChildOfGroup && a._4 == h) || (h == null)) {
            q.removeChild(c);
            r._3 = null;
            return
        }
...
OrgChart.prototype._aj = function(c, b) {
...
OrgChart.prototype._av = function(b, a) {
    a.stopPropagation();
    a.preventDefault()
};
OrgChart.prototype._am = function(c, b, a) {
    this.menuUI.hide();
    var d = BALKANGraph._at(b, a, this.getSvg());
...
OrgChart.prototype._ar = function() {
    var a = new BALKANGraph.node(BALKANGraph._al(), null, null, [], this.config.template);
    this.addNode(a);
    this.center(a.id)
};
OrgChart.prototype.zoom = function(b, a) {
    ...
    h[0] = d[0] - (h[2] - f) / (100 / a[0]);
    h[1] = d[1] - (h[3] - e) / (100 / a[1]);
    var c = this.getScale(h);
    h[2] = this.width() / c;
    h[3] = this.height() / c;
    if (((b === true) && (c < this.config.scaleMax)) || ((b === false) && (c > this.config.scaleMin)) || (b != false && b != true && c < this.config.scaleMax && c > this.config.scaleMin)) {
        ...
        var g = this;
        setTimeout(function() {
            g._l(true, BALKANGraph.action.zoom)
        }, 500)
    }
};
